...
class TestClipCaptionModel(nn.Module):
    ...
    def __init__(
        ...
        # Save GPT2Tokenizer with added special EOS token.
...
class TestCaptionDecoder(object):
    def __init__(
        self,
        device,
        tokenizer_pretrained_path,
...
def evaluate(config):
    ...
    if use_caption_decoder:
        ...
        caption_decoder = TestCaptionDecoder(device=device, **config.caption_decoder)
    else:
...
config_flags.DEFINE_config_file(
    "config", "configs/sample_test_model.py", "Configuration.", lock_config=False)
